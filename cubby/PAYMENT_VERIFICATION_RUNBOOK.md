...
## 0. Two gaps discovered while compiling this runbook — both now resolved

Writing this runbook required tracing the full payment chain end-to-end, including the parts outside the five paygate-* functions themselves. Two real gaps surfaced that didn't belong buried in a checklist — both are now fixed and merged to `main`:

1. **The traveller app didn't call any paygate-* function.** `app/(traveller)/booking.tsx` called `payfast-create`. **Fixed in PR #71** — `booking.tsx` now calls `paygate-initiate`/builds the `paygate-redirect` URL, with the old `payfast-create` call fully removed (not branched around). A follow-up final review on that same PR also found and fixed a deeper, related trust gap: the deep-link return handling (both `booking.tsx`'s local handling and a separate, pre-existing global fallback handler in `app/_layout.tsx` used when the app is backgrounded during checkout) treated the browser's returned `status` param as proof of payment. Both now re-fetch the booking's real status from Supabase (`src/lib/payment-status.ts`) before ever showing a "confirmed" screen — see Section 8's deep-link row and Section 9.
2. **Host and traveller were never notified when a PayGate payment confirms a booking.** `_shared/awaiting-host-notifications.ts` — the one shared function that sends the "payment received, waiting on host" / "new booking request" in-app, push, and email notifications after a fresh `confirm_booking_payment` success — was called by `payfast-itn`, `payment-webhook`, and `payment-result`, but not `paygate-notify` or `paygate-query`. **Fixed in PR #72** — both now call the same shared helper, fire-and-forget, strictly inside the branch reached only on a fresh `confirm_booking_payment` success (never on `already_resolved`, an RPC error, or any rejection path), so a notification failure can never affect the already-acknowledged payment callback.

Both PRs are merged to `main` (2026-07-31). **Deploying these changes** (Edge Functions per Section 3, and the app build itself) **is still a separate, pending step** — see Section 9.
...
| `PAYGATE_ENCRYPTION_KEY` | same | The paired encryption key for whichever `PAYGATE_ID` is set above. A mismatched pair doesn't error visibly — it just makes every checksum verification fail (see Section 8, "`PAYGATE_ID`/`PAYGATE_ENCRYPTION_KEY` misconfigured or mismatched pair"). |
...
Now that Gap #1 (Section 0) is fixed, this **can** be driven by tapping through the app's real booking flow once PR #71 is deployed — do that first as the primary path, since it's what a real traveller will actually do. The manual `curl` path below remains useful for isolating a specific step (e.g. testing `paygate-query` in isolation, which nothing in the app calls automatically) or if app deployment is lagging behind function deployment:
...
**Gap #2 from Section 0 is fixed (PR #72):** the transition into `awaiting_host_confirmation` via a PayGate payment now triggers the same host/traveller notifications the same transition via PayFast/Peach already did, via the shared `sendAwaitingHostNotifications` helper.
...
| `paygate-notify` never arrives at all | Booking sits in `pending_payment` indefinitely (not `awaiting_host_confirmation` — there's no partial state). Use `paygate-query` to reconcile — nothing in the app calls it automatically yet (no scheduled sweep exists), so it must still be triggered manually or via the `x-admin-secret` server-to-server path. | Section 5's reconciliation-candidates query |
...
| A booking is confirmed via PayGate and the host is not notified | **Should not happen post-PR #72** — `sendAwaitingHostNotifications` is called on every fresh confirm via both `paygate-notify` and `paygate-query`. If seen after both PRs are deployed, check `[paygate-notify] Notification error:` / `[paygate-query] Notification error:` in logs (Section 7) — the call is fire-and-forget, so a failure here never blocks the payment confirmation itself, but it does mean the notification genuinely didn't go out and is worth investigating (e.g. `send-push`/`send-email`'s own secrets, per Section 1). | Section 7, `PROJECT_MASTER_PLAN.md` |
| The app shows "Payment successful" for a booking that isn't actually confirmed yet | **Should not happen post-PR #71's final review round** — neither `booking.tsx`'s local deep-link handling nor `app/_layout.tsx`'s global fallback (used when the app is backgrounded during checkout) ever shows a success state without first re-fetching the booking's real status from Supabase (`src/lib/payment-status.ts`). If seen, this is a genuine regression, not a known limitation — treat it as a real bug. | `booking.tsx`, `app/_layout.tsx`, `payment-success.tsx` |
...
- [x] **Gap #1 (Section 0) resolved**: `booking.tsx` calls `paygate-initiate`/`paygate-redirect` instead of `payfast-create` (PR #71, merged to `main` 2026-07-31).
