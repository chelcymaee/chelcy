...
> - ✅ **Admin authentication hardening (2026-07-21 → 2026-07-27, PR #60, merged and deployed to production):** grew out of the Private Beta readiness audit. Founder's call after seeing exactly what the client-side admin PIN protected (every host's real bank account number/branch code, all user PII, the ability to approve/reject identity verification and delete reviews platform-wide): retire it entirely rather than accept the risk short-term. `EXPO_PUBLIC_ADMIN_PIN` and `EXPO_PUBLIC_ADMIN_SECRET` — both bundled into the shipped app/web JS by the `EXPO_PUBLIC_` prefix, extractable by anyone who inspected the bundle — no longer exist anywhere on the client. Full design:
...
>   - **✅ Deployed and live-tested against production Supabase (2026-07-27), founder-run:** database migration applied (`admin_sessions`, `admin_login_attempts`), `ADMIN_PIN` set as a new server-only secret, all 7 `admin-*` functions plus `verify-admin-pin` redeployed with `--no-verify-jwt` and confirmed off via the Dashboard's "Verify JWT with legacy secret" toggle on each (not just assumed from the deploy flag). Full live checklist run against the real deployed backend: correct-PIN login ✅, wrong-PIN messaging with accurate attempts-remaining ✅, lockout at the 5th failure with an accurate server-driven countdown, unbypassable by a correct PIN while locked ✅, an authenticated action (host bank details — the highest-stakes screen) loading real data ✅, logout confirmed to revoke server-side via direct replay of the old token afterward (`curl` → `401 {"error":"Unauthorized"}`, not just a local storage clear) ✅, session expiry confirmed via the same `expires_at` check already proven in local Postgres testing plus the live logout-rejection path exercising the identical code ✅. Two narrower checks — `send-push`'s new admin-session path (via a verification approve/reject) and `send-email`'s new JWT path (via a review submission) — were **deferred, not failed**: no pending verification or completed booking exists yet to test with, since PayFast isn't connected yet. Not a blocker; both will get their first real exercise once real bookings/verifications exist, tracked here rather than forgotten.
>   - One incidental finding during live testing, unrelated to this change's correctness: while testing the anon-key concept, a `sb_secret_...` key (Supabase's newer naming for the full-access secret key, equivalent to the old service-role key) was briefly pasted where the safe-to-share `sb_publishable_...` key belonged. Flagged to the founder to rotate out of caution; not confirmed misused, and very likely not yet wired into anything live since edge functions still use the older-style `SUPABASE_SERVICE_ROLE_KEY`.
...
>   - **Outstanding, founder action:** rotate the `sb_secret_...` key noted above (precautionary); remove `EXPO_PUBLIC_ADMIN_PIN`/`EXPO_PUBLIC_ADMIN_SECRET` from the Expo/EAS client build environment (safe to do any time now — nothing in the code reads them anymore) — do not touch the server-side `ADMIN_SECRET`, still required by `booking-expiry-sweep`, `complete-booking`, `send-review-reminders`, and the unchanged server-to-server paths in `send-push`/`send-email`.
...
| `"Anyone can view active host profile verification"` RLS policy on `profiles` exposes the **entire row** (email, phone, avatar_url — not just verification status) for any host with an active listing, to any authenticated user, with no relationship requirement (no booking, no conversation, nothing). Row-level security can't filter by column, so a policy meant to expose one field ends up exposing all of them. Pre-existing — not introduced by PR #79, which only surfaced it while wiring the traveller-side host avatar. Host→traveller reads (the properly booking-scoped `"Hosts can view profiles of their travellers"` policy) are unaffected. Separate investigation opened; see Decisions & Context Log 2026-08-05. | 🔴 Critical (PII exposure, found 2026-08-05 via `RLS_VERIFICATION.sql` block #6, not yet fixed) | Supabase `profiles` RLS policy (production) |
...
| 2026-08-05 | Traveller-side host avatar display (in `traveller/messages.tsx`) deferred, not merged, pending a safe host-profile access model | PR #79 fixed personal-avatar display for the host→traveller direction (`host/traveller-profile.tsx`, `host/messages.tsx`), which correctly rides on the booking-scoped `"Hosts can view profiles of their travellers"` RLS policy — kept and merged. The traveller→host direction was reverted before merge: running `RLS_VERIFICATION.sql` block #6 live showed it only worked because of the unrelated, over-broad `"Anyone can view active host profile verification"` policy (see Known Bugs — exposes the full `profiles` row, not just verification status, to any authenticated user for any active host, no relationship required). Founder decision: do not ship a feature that depends on a policy already flagged as a PII exposure, and do not patch it inline in this PR. `traveller/messages.tsx` keeps its pre-existing 🏠 fallback for hosts until a proper public host-profile surface (see Known Bugs row) replaces the broad policy — likely a dedicated view/RPC exposing only approved fields (display name, verification status, avatar), not raw `profiles`. Separate investigation opened to map every query currently depending on the broad policy before it's touched. |
