...
## 🧹 PRIVATE BETA POLISH — Sprint 4 🟡 In progress (Priority 4 blocked, awaiting decision)
...
| First-time host onboarding checklist | 🔴 Blocked — architectural discovery, needs a decision | While building this, found that **the self-service host listing flow may not actually work at all**: `app/(host)/host-profile.tsx`'s save function is `.update(...).eq('assigned_user_id', user.id)` — an UPDATE, never an INSERT. If a traveller self-registers choosing the "host" or "both" role (which `signup.tsx` fully supports and routes straight to bank details), they have no `hosts` row yet, so this update silently matches zero rows and appears to succeed while creating nothing. Separately, `app/(host)/dashboard.tsx` shows a real bug in the same area: when a host has no `hosts` row, it displays an error banner ("Host profile not found") **and simultaneously still renders the hardcoded `DEMO_STATS` fallback** (R1,240 earnings, 14 bookings, two fake "Sarah T." / "James M." bookings) below it, since the early-return never clears the initial demo state. This matters directly for the checklist: the master plan's own decision log says hosts are meant to be admin-created then assigned (`assigned_user_id` pattern, "allows quality control before self-serve") — which conflicts with `signup.tsx` publicly offering self-service host signup. Building a checklist that tells a self-registered host to "Create listing" would point at a capability that doesn't actually exist yet. **Did not implement anything for this item — stopped to ask which model is correct before building the wrong thing.** See question to founder. |

**Verified so far:** `tsc --noEmit` — zero new errors from any Sprint 4 change (same pre-existing unrelated errors as before). Playwright headless test confirms the error boundary and both Runners fixes work live.
