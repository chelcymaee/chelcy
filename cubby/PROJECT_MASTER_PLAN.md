...
| No booking capacity/overlap enforcement | ✅ Fixed | Confirmed genuine: `booking.tsx`'s `handleConfirm` inserted directly with no check against the host's `max_bags`. Multiple travellers could book the same host past capacity with zero warning to either side. **Fix:** added a same-day bag-count check (sums `bag_count` across `pending_payment`/`pending`/`confirmed`/`active` bookings for that host + date, blocks if adding this booking would exceed `max_bags`, shows the traveller how many spaces are actually left). Deliberately day-level, not full time-interval overlap — appropriate for private beta scale, avoids adding real complexity for marginal benefit right now. |
| `messages`/`conversations` RLS: enabled but zero policies committed anywhere in the repo | ✅ Fix written, needs live verification + apply | Confirmed: `schema.sql` shows `ALTER TABLE messages/conversations ENABLE ROW LEVEL SECURITY` with no `CREATE POLICY` anywhere in the repo. `chat.tsx` derives conversation participants from a client-supplied `bookingId` with **no ownership check of its own** — it relies entirely on RLS being correct, and RLS correctness here is completely unverifiable from this sandbox (no live DB access). **Added:** "Fix 4" block to `schema.sql` with explicit participant-only policies for SELECT/INSERT/UPDATE on both tables, safe to run even if different-named policies already exist live. **Not yet applied to the live project** — needs to be run in the SQL Editor, and verified with `supabase/RLS_VERIFICATION.sql` (new file, ready-to-run diagnostic queries, same method as the real Sprint 3 bug discovery) before and after. |
| `hosts`, `reviews`, `bookings` tables: RLS policies not in version control at all | ⚠️ Needs live verification | These three tables aren't even `CREATE TABLE`'d in `schema.sql` — they were created directly in the Supabase Dashboard at project inception, so their RLS is completely opaque to static analysis. The admin-panel access paths for `hosts`/`reviews` are now safe regardless (routed through `ADMIN_SECRET`-gated edge functions since the previous Sprint 5 pass), but whether a *regular* signed-in user could directly `INSERT` a fake host, `DELETE` someone else's review, or `SELECT` other people's bookings via the direct table API is unknown. `RLS_VERIFICATION.sql` includes ready-to-run checks for all three. |
