...
- [x] `paygate-return` edge function — browser return handler, read-only/non-authoritative. See "paygate-return design decisions" below for the IDOR found and fixed during review. `query.trans` reconciliation remains a separate, not-yet-started follow-up (PR #68)
...
- The `query.trans` reconciliation fallback (used when notify/return never arrive) can only query the *most recently stored* attempt — an older completed-but-unnotified attempt would not be reconcilable that way. Same limitation as before, now the shared reason both `paygate-return`'s cosmetic gap and this one exist.
...
`query.trans` reconciliation remains a separate, not-yet-started follow-up — kept explicitly apart from `paygate-return` so the browser return path never becomes authoritative for confirming a payment.
