...
>
> **Live RLS verification is in progress (2026-07-07), run directly against production via `RLS_VERIFICATION.sql`:** block #2 (messaging cross-user access) came back **safe** — 0 rows visible to a non-participant. Block #1 (anonymous read) came back **safe** — 0 rows for `anon` role on both tables. Block #3 (fake host creation) came back **unsafe, confirmed and fixed**: a real non-host traveller account could INSERT directly into `hosts`, bypassing admin approval entirely — root cause was `"Hosts can manage own listing"` being a `FOR ALL` policy whose `USING` clause doubled as an implicit `WITH CHECK` on INSERT. Fix is written as "Fix 5 (Sprint 5 launch audit)" at the end of `schema.sql` (drops the `FOR ALL` policy, replaces with UPDATE/DELETE-only self-service policies, adds no INSERT policy) — **not yet applied to the live database**, needs to be run in the SQL Editor, then block #3 re-run to confirm closed. Blocks #4 (review deletion) and #5 (booking visibility) not yet run.
>
...
> 2. **Apply the "Fix 5" hosts RLS policy** (end of `schema.sql`) in the Supabase SQL Editor, then re-run `RLS_VERIFICATION.sql` block #3 to confirm the INSERT now fails with a policy violation.
> 3. **Finish `supabase/RLS_VERIFICATION.sql`** — blocks #4 (review deletion) and #5 (booking visibility) still need to be run; report results back. If either comes back unsafe, write and apply the matching fix the same way as Fix 5. Block #2 (messaging) is confirmed safe — the candidate "Fix 4" policies in `schema.sql` are not needed and should stay unapplied.
