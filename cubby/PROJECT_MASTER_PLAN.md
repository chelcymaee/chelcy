...
> - ⏳ **Phases 2-7 not started.** Do not activate the new `awaiting_host_confirmation` state (Phase 5, the `payfast-itn` cutover) until Accept, Decline, and Expiry all work end-to-end (Phase 4) — a paid booking must never be able to enter a state with no tested way out.
>
