...
| `hosts`, `reviews`, `bookings` tables: RLS policies not in version control | ⚠️ Open verification question — SQL ready, not yet run | Same caveat as above: these three tables aren't `CREATE TABLE`'d in this repo at all (created directly in the Supabase Dashboard at project inception), so their live RLS state is simply unknown from here — not assumed broken, not assumed safe. `RLS_VERIFICATION.sql` has the exact checks to run. The admin-panel access paths for `hosts`/`reviews` are already safe regardless of the outcome (routed through `ADMIN_SECRET`-gated edge functions since the previous Sprint 5 pass) — this only concerns the *direct table* access path for regular signed-in users. |
