...
| `reviews`, `bookings` tables: RLS policies not in version control | ⚠️ Open verification question — SQL ready, not yet run | These two tables aren't `CREATE TABLE`'d in this repo at all (created directly in the Supabase Dashboard at project inception), so their live RLS state is unknown from here — not assumed broken, not assumed safe, especially now that the `hosts` row above turned out to have a real committed policy with a subtle bug rather than no policy at all, so no assumption should be made either way for these two. `RLS_VERIFICATION.sql` blocks #4 (review deletion) and #5 (booking visibility) have the exact checks to run — in progress. The admin-panel access paths for `reviews` are already safe regardless of the outcome (routed through the `ADMIN_SECRET`-gated `admin-reviews` edge function) — this only concerns the *direct table* access path for regular signed-in users. |
...
>
> **Live RLS verification is in progress (2026-07-07 → 2026-07-08), run directly against production via `RLS_VERIFICATION.sql`:** block #2 (messaging cross-user access) came back **safe** — 0 rows visible to a non-participant. Block #1 (anonymous read) came back **safe** — 0 rows for `anon` role on both tables. Block #3 (fake host creation) came back **unsafe, confirmed and fixed** — took two fixes to actually close: the first ("Fix 5" — the `"Hosts can manage own listing"` `FOR ALL` policy implicitly allowing self-service INSERT) turned out not to be sufficient on its own, because a second, undocumented policy (`"Admin can manage all hosts"`, added directly in the Dashboard, `USING (true)`/`WITH CHECK (true)` for every role) was independently granting the same access. Found via a live `pg_policies` query after Fix 5 alone didn't close the test. "Fix 6" dropped that policy after migrating its one legitimate dependent (`verifications.tsx`'s verified-badge sync) onto the service-role `admin-hosts` edge function. Both fixes are now **applied to the live database** — block #3 re-run and confirmed closed (`ERROR: 42501: new row violates row-level security policy for table "hosts"`). Blocks #4 (review deletion) and #5 (booking visibility) not yet run.
>
> Remaining known manual items before Private Beta (founder's explicit next priority — deploy + verify, no new feature work):
> 1. **Deploy the 5 edge functions touched in PR #40, plus redeploy `admin-hosts`** (gained `owner_is_verified` in its field allowlist as part of the Fix 6 migration) — exact commands below.
> 2. **Manually verify the `verifications.tsx` approve/reject flow** still updates a host's verified badge correctly now that it's routed through `admin-hosts` — this sandbox can't reach production to test it.
> 3. **Finish `supabase/RLS_VERIFICATION.sql`** — blocks #4 (review deletion) and #5 (booking visibility) still need to be run; report results back. If either comes back unsafe, write and apply the matching fix the same way as Fix 5/6 — and given what block #3 just taught us, check `pg_policies` directly for the relevant table rather than assuming `schema.sql` has the full picture. Block #2 (messaging) is confirmed safe — the candidate "Fix 4" policies in `schema.sql` are not needed and should stay unapplied.
> 4. Re-enable Supabase email confirmation (Dashboard toggle) — still pending
> 5. Decide whether to drop the orphaned `bank_details` table (see Sprint 4 audit note) — recommend dropping after confirming it holds no live data
> 6. Once PayFast is actually configured: run a dedicated payment QA sprint (see Payment QA Checklist in Final QA section) — explicitly deferred until real transactions are possible
> 7. Follow up on the flagged-but-unverified `profiles` RLS question in `verifications.tsx` (see Final QA section row above)
> 8. Everything else in the Private Beta / Public Beta checklists below
>
